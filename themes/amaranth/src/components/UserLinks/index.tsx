...
import { Twitter, Instagram,Tiktok, LinkedinSquare, Github } from "@styled-icons/boxicons-logos";
...
const renderInstagramLink = (
  ...
  const userName = config.user?.instagramName;
...
const renderTikTokLink = (
  config: Readonly<SiteConfig>
): JSX.Element | null => {
  const userName = config.user?.tiktokName;

  if (!userName) return null;

  const url = `https://tiktok.com/${userName}`;
  return renderLink(url, "TikTok Profile", Tiktok);
};
...
const UserLinks = ({
  ...
  return (
    <LinkGrid className={className}>
      ...
      {renderInstagramLink(config)} {renderTikTokLink(config)}
